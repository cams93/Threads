...
for(t=0;t<NUM_THREADS;t++) {
  sum = sum + t;
  thread_data_shared.thread_id = t;
  thread_data_shared.sum = sum;
  thread_data_shared.message = messages[t];
  printf("Creating thread %d\n", t);
  rc = pthread_create(&threads[t], NULL, PrintHello,  (void *)&thread_data_shared);
  pthread_mutex_lock (&mutexsum); 
  //sleep(1);
  if (rc) {
